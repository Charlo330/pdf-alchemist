...
export class FilePickerModal extends Modal {
	...
	private async loadAllLinks(): Promise<void> {
		try {
			...
			if (await this.app.vault.adapter.exists(indexPath)) {
				...
				this.allLinks = Object.entries(index).map(
					([pdfPath, properties]) => {
	...
	private displayCurrentPage(): void {
		...
		resultsInfo.textContent = `Showing ${startIndex + 1}-${endIndex} of ${
			this.filteredLinks.length
		} links`;
	}
	...
	private createLinkItem(link: LinkItem, index: number): void {
		...
		const noteDiv = linkInfo.createEl("div", {
			cls: "link-path note-path",
		});
	...
	private updatePagination(): void {
		...
		const totalPages = Math.ceil(
			this.filteredLinks.length / this.itemsPerPage
		);
